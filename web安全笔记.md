...
对于CSP的default-src 'none'情况，网页隔绝了所有资源的加载，（即img，css，js），此时不管用，但仍然可以使用a标签的引用，我们可以通过base标签的target属性，或是form标签的target，不闭合它，设置网页窗口为target后面的内容，然后诱导用户点击跳转到第三方网站，恶意网站读取窗口名字，来获取用户的csrf token.
...
万能测试XSS漏洞：```"/></text><script>alert(1)</script>```

新思路：修改的参数可能不会在页面显示，但是故意使参数类型报错，这个报错信息可能会显示到页面上，进而触发
